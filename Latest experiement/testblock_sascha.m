...
function trialmtx = testblock_sascha(data, nBlocks)
...
allstim = [data.picprob(:,[1 3 5]); data.picprob(:,[2 4 6])];
...
% % keep only the rows that compare stimuli with equal probabilities or from
% the same block
keyComparisons = [];
    for n = 1:size(alltest_t1,1)
        if alltest_t1(n,3) == alltest_t1(n,4) || alltest_t1(n,5) == alltest_t1(n,6)
            keyComparisons(end+1,:) = alltest_t1(n,:);
        end
    end
...
% if nBlocks == 2
    ...
    alltest_t1 = alltest_t1(randperm(size(keyComparisons,1)),:); % Shuffle test stimuli
...
starttime=time;
cgfont('Arial',data.FontSizeText); cgpencol(1,1,1); cgflip(data.background)
cgtext('Great job so far!',0,300);
cgtext('Now we will show you the animals from the first two rounds again.',0,200);
cgtext('Think well before you choose, you will be shown each pair only once.',0,150);
cgtext('Points in this round count double.',0,100);
cgtext('So you can earn a lot of extra points if you do well.',0,-50);
...
cgflip(data.background);wait(data.time.delay);
...
for itrial=1:size(trialmtx,1)
   ...
   %cgtext('Think well before you choose.',0,300);
   cgtext('You will see each pair only once.',0,200);
   cgtext('Please wait a second before giving your rating.',0,-200);
   ...
   %cgfont('Arial',data.FontSizeTask); cgtext('+',0,0);    
    ...
    if itrial>ceil( size(trialmtx,1)/2 ) 
        ...
        originy = -200; sx = [-200-stimDimensions(1)/2 200+stimDimensions(1)/2]; sy = [originy, originy]; % for lefthandside
        timelimit=Inf;
